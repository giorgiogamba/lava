...
std::unique_ptr<LavaModel> Application::createCubeModel(LavaDevice& device, glm::vec3 offset) {
    ...
    return std::make_unique<LavaModel>(device, Builder);
}

void Application::LoadGameObjects()
{
    const std::shared_ptr<LavaModel> Model = createCubeModel(Device, {.0f, .0f, .0f});
    LavaGameObject Cube = LavaGameObject::CreateGameObject();
    Cube.SetModel(Model);
