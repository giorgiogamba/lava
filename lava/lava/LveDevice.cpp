...
LveDevice::LveDevice(LveWindow &window) : window{window}
{
    ...
    // Enables validation layers because Vulkan is very error prone.
    ...
    // Connects to the logical device representation
    // Descrubes the physical functions we want to use from GPU
    createLogicalDevice();
    createCommandPool();
}
