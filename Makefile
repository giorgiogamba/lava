...
CC_FLAGS = -std=c++17 \
		   -I $(INC_DIR) \
		   -I ${USER_PATHS}/include \
		   -I $(VULKAN_SDK_PATH)/include \

LD_FLAGS = -L ${USER_PATHS}/lib \
		   -L $(VULKAN_SDK_PATH)/lib -lglfw.3 -lvulkan \
		   -F $(USER_PATHS)/lib -framework QtWidgets -framework QtGui -framework QtCore \
		   -lpthread \
